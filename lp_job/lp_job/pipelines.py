...
import pandas as pd
import pymysql, time, redis, json, uuid
...
class JobCardPipeline:
    ...
    def close_spider(self, spider):
        ...
        # mysql数据库
        mysql_ins = pymysql.connect(host=settings.get('MYSQL_HOST'),port=settings.get('MYSQL_PORT'),user=settings.get('MYSQL_USER'),password=settings.get('MYSQL_PASSWORD'),database=settings.get('MYSQL_DATABASE'))
        mysql_cursor = mysql_ins.cursor()

        # mysql写入部分
        # cmp写入
        cmp_sql = "INSERT INTO COMPANY (CMP_ID,CMP_NAME,CMP_SCALE_MIN,CMP_SCALE_MAX,CMP_LINK,CMP_STAGE,CMP_LOGO,CMP_INDUSTRY) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"
        mysql_cursor.executemany(cmp_sql,self.cmp_list)
        mysql_ins.commit()

        # rec写入
        rec_sql = "INSERT INTO RECRUITER (REC_ID,IM_ID,IM_USERTYPE,REC_NAME,REC_TITLE,REC_PIC) VALUES (%s,%s,%s,%s,%s,%s)"
        mysql_cursor.executemany(rec_sql,self.rec_list)
        mysql_ins.commit()
        ...
        #label写入
        label_sql = "INSERT INTO JOBLABEL (LABEL_ID,LABEL_NAME) VALUES (%s,%s)"
        mysql_cursor.executemany(label_sql,self.label_list)
        mysql_ins.commit()

        # joblabel写入
        jl_sql = "INSERT INTO CARDLABEL (JOBCARD_ID_ID,LABEL_ID_ID) VALUES (%s,%s)"
        mysql_cursor.executemany(jl_sql,self.jl_list)
        mysql_ins.commit()
